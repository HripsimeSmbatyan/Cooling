...
//the buildings appear only when there are walls, as we will see later	 
 ...
 const updateenemyshoot=function()//updateballs
 {
	...
	enemy[i].isok=true;
	 ...
	 enemy[i].enshoot[enemy[i].enshoot.length]=gamedata.enemyinfo.enshootinfo;//a new ball is created
...
document.addEventListener("keydown", function(event){
	 ...
	 {
	 initial=hero.x;//it depends from this
	...
	y	:hero.y+50*a/1000,//the ball begins on the nose of our hero
	r	:a/200, //radius
	xdelta	:15,//speed
...
const updateweapon=function()//our weapon
{
	...
	{
	if(hw[hw.length-1].x-initial>=a/4)
	ispossible=true;
	else
	ispossible=false;
	}
 ...
 const updateboss=function()
 {
	...
	{
	bossvis=10;   //if the collision is in the process, it does not take more lives
	life=1;	
	}
	if(life===2)
	{
...
const updatebossshoot=function()
{
 ...
 const updatewallsceiling=function()
 {
	...
	arrwallsceiling[i].x-=arrwallsceiling[i].xdelta;//the movement of walls 
